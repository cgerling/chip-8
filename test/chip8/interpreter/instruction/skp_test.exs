defmodule Chip8.Interpreter.Instruction.SKPTest do
  use ExUnit.Case, async: true

  alias Chip8.Interpreter
  alias Chip8.Interpreter.Instruction.Argument.Register
  alias Chip8.Interpreter.Instruction.SKP
  alias Chip8.Interpreter.VRegisters

  describe "execute/2" do
    test "should return an interpreter with pc set to next instruction when key of vx is pressed" do
      ...
      vx = %Register{value: x}
      arguments = {vx}
      executed_interpreter = SKP.execute(interpreter, arguments)

      assert %Interpreter{} = executed_interpreter
      assert interpreter.pc + 2 == executed_interpreter.pc
    end

    test "should return an interpreter with pc unchanged when key of vx is not pressed" do
      interpreter = Interpreter.new()
      x = :rand.uniform(0xF)
      x_value = :rand.uniform(0xF)
      v_registers = VRegisters.set(interpreter.v, x, x_value)
